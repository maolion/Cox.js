...
;void function __Cox_Base__(){
    ...
    void function __Cox_Module_Manage__(){
        ...
        /**
         * getModuleId 从模块的url中解析一串字符作为模块的id
         * @param { String } url
         * @return{ String }
         */
        function getModuleId( url ){
            return url.replace( /\?\d+\&/, "?" ).replace( "?#", "#" ).replace( /(?:\.js)?(?:\?\d+)?$/, "" );
        }
        /**
         * getModuleUrl 计算一完整的URL
         * @param { String } uri 相对的路径 
         *  如果 uri 是以 '/' 字符开头，则其根目录为 LOCA_ROOT 表示的值
         *  如果 uri 是以 '@' 字符开关，则返回不包含 '@' 的 uri
         * @param { String } root 相同路径根目录
         * @return{ String }
         *  如果 config.debug 是一个真值那么 getModuleUrl返回的URL会在查询参数列表中添加一串唯一性的字符串
         */
        function getModuleUrl( uri, root ){
            var 
                newtime  = !config.debug ? "" : "?" + new Date().getTime(),
                isurl    = RE_PROTOCOL_SIGN.test( uri ),
                url      = null,
            ...
            if( isurl ){
                ...
                switch( uri.slice( 0, 2 ) === "~/" && "~/" || uri.charAt(0) ){
                    case "~/":
                        root = "~/";
            ...
            if( url.indexOf("?") === -1 && url.indexOf("#") === -1 ){
                ...
                url = url.replace( RE_URL_PARAMS, newtime + "&$1$2" );
            }
            url = !isurl ? protocol  + url : url;
            return url;
        };
        ...
        function loadModule( module ){
            ...
            loader.setAttribute( "src"    , module.url );
        ...
        function _require( tmodule ){
            var tdepend = tmodule.depend;
            return function require( uri ){
                var 
                    module = tdepend.reallist[ uri ] || 
                             tdepend.reallist[ getModuleId( getModuleUrl( 
                                uri, 
                                dirname( tmodule.url ) || config.moduleroot 
                             ) ) ],
                ...
                if( module.state !== MODULE_DEFINED && typeof module.__DEFINE__ === "function" ){
                    try{
                        ...
                        define.call( null, _require( module ), module.exports, module );
                    }catch( e ){
        ...
        if( LOCA_URL.length ){
            ...
            getModuleUrl  = function( uri, root ){
                var root = root || "";
                switch( uri.slice( 0, 2 ) === "~/" && "~/" || uri.charAt(0) ){
                    case "~/":
                        root = "~/";
                    ...
                    case "@":
                        root = "";
                        return realpath( uri.slice(1) ).slice(1);
                    break;
                }
            ...
            loadModule    = function( module ){
                ...
                try{
                    load = REQUIRE( module.url || module.id );
                }catch( e ){
                    module.state = MODULE_LOAD_FAIL;
                    _Modules.fireEvent( "error", [ module ] ); 
                    console.log( "load fail:", module.id );
        ...
        config.roots[ "~/Cox" ] = MODULE_ROOT + "/modules";
        ...
        Module = _Class( "Module", _Finaly, null, null, function ( Module, Public ){
            ...
            Public.setUri = function( uri, root ){
                ...
                this.uri  = uri;
                if( uri.charAt( 0 ) !== "@" ){
                    this.url  =  uri && getModuleUrl( uri, root );
                    this.id   = uri && getModuleId( this.url );
                }else{
                    this.url  = "";
                    this.id   = uri.slice( 1 );
                }
            ...
            _Util.enumDict( loadingModules, true, function( module, id ){
                if( module instanceof Module ){
                    if( module.branch && module.branch[ modulename ] instanceof Module ){
                        ...
                        if( id.lastIndexOf( cmodulename ) - id.length + cmodulename.length === 0 ){
                            newmodule = module;
                            return false;
                        }
                    }
        ...
        _UTest.add( "Cox.Module_Manage.funs", function test_funs(){
            ...
            this.assert( getModuleId( "a" ) === "a", 'getModuleId( "a" ) === "a"' );
            this.assert( getModuleId( "a/b/c.js" ) === "a/b/c", 'getModuleId( "a/b/c.js" ) === "a/b/c"' );
            this.assert( getModuleId( "http://test.com/x" ) === "http://test.com/x", '"http://test.com/x" ) === "http://test.com/x"' );
            this.assert( getModuleId( 
                "http://test.com/x.js?232323232323&v=30" ) === "http://test.com/x.js?v=30", 
                '"http://test.com/x.js?232323232323&v=30" ) === "http://test.com/x.js?v=30"'
            );
            this.assert( getModuleId( 
                "http://test.com/x.js?232323232323" ) === "http://test.com/x", 
                '"http://test.com/x.js?232323232323" ) === "http://test.com/x"'
            );

            this.assert(getModuleUrl( "~/modules2/a" ) );
            this.assert(getModuleUrl( "~/Cox/a" ) );
            this.assert(
                getModuleUrl( "@A" ) === "A", 'getModuleUrl( "@A" ) === "A"'
            );
            ...
            var 
                root1 = config.roots["~/"] + "/" + "modules",
                root2 = "http://test.com/modules",
                debug = config.debug
            ;
            config.debug = false;
            this.assert( 
                getModuleUrl( "A" ) === config.roots["~/"] + "/" + "A.js", 
                'getModuleUrl( "A" ) === "' +  config.roots["~/"] + "/" + "A.js" + '"'
            );
            this.assert( 
                getModuleUrl( "A", "" ) === "A.js", 
            ...
            this.assert( 
                getModuleUrl( "A", root1 ) === root1 + "/" + "A.js", 
                'getModuleUrl( "A", root1 ) === "' +  root1 + "/" + "A.js" + '"'
            );
            this.assert( 
                getModuleUrl( "/A" ) === LOCA_ROOT + "/" + "A.js", 
                'getModuleUrl( "/A" ) === "' +  LOCA_ROOT + "/" + "A.js" + '"'
            );
            this.assert( 
                getModuleUrl( "/A", root1 ) === LOCA_ROOT + "/" + "A.js", 
                'getModuleUrl( "/A", root1 ) === "' +  LOCA_ROOT + "/" + "A.js" + '"'
            );

            this.assert( 
                getModuleUrl( "http://test.com/A" ) === "http://test.com/A",  
                'getModuleUrl( "http://test.com/A" ) === "http://test.com/A"'
            );

            this.assert( 
                getModuleUrl( "A", root2 ) === root2 + "/A.js",  
                'getModuleUrl( "A", root2 ) === "' + root2 + '/A.js"'
            );
            ...
            var 
                url1 = getModuleUrl( "A" ),
                url2 = "http://test.com/A",
                url3 = "http://test.com/A.js",
                url4 = "http://test.com/A.js?v=0#100"
            ;
            config.debug = true;

            this.assert( getModuleUrl( "A" ) );
            this.assert( getModuleUrl( "A", root1 ) );
            this.assert( getModuleUrl( "A", root2 ) );
            this.assert( getModuleUrl( "/A" ) );
            this.assert( getModuleUrl( url2 ) );
            this.assert( getModuleUrl( url3 ) );
            this.assert( getModuleUrl( url4 ) );
